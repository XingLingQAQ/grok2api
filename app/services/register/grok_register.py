...
class GrokRegister:
    ...
    async def _fetch_action_id(self, client: httpx.AsyncClient) -> Optional[str]:
        """从注册页面获取 action_id"""
        try:
            res = await client.get(f"{SITE_URL}/sign-up", timeout=15.0)
            ...
            for js_url in js_urls:
                ...
                try:
                    js_res = await client.get(js_url, timeout=10.0)
    ...
    async def _send_verification_code(
        ...
        try:
            res = await client.post(url, content=data, headers=headers, timeout=15.0)
    ...
    async def _verify_code(
        self, client: httpx.AsyncClient, email: str, code: str
    ) -> bool:
        """验证邮箱验证码"""
    ...
    async def _submit_registration(
        ...
        try:
            ...
            await client.get(verify_url, follow_redirects=True, timeout=15.0)

            # 获取 SSO Token
            sso = client.cookies.get("sso")
            return sso
    ...
    async def register_single(self) -> RegisterResult:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                # Step 0: 获取 action_id（如果还没有）
                if not self._action_id:
                    self._emit("fetching_action_id")
                    self._action_id = await self._fetch_action_id(client)
                    if not self._action_id:
                ...
                if not await self._send_verification_code(client, email):
                    return RegisterResult(
                ...
                if not await self._verify_code(client, email, verify_code):
                    return RegisterResult(
                ...
                # Step 5: 获取 Turnstile Token（最多重试 3 次）
                for captcha_attempt in range(3):
                    ...
                    sso_token = await self._submit_registration(
                        client, email, password, verify_code, turnstile_token
                    )
